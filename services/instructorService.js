...
class InstructorService {
  ...
  static async createInstructor(instructor) {
    try {
      ...
      // The check below is redundant if isInstructorExistByUserId is already done
  ...
  /**
   * Retrieves an instructor by their ID (primary key of instructors table).
   *
  ...
  /**
   * @async
   * @description Retrieves an instructor by their User ID (foreign key to users table).
   * @param {number} userId - The user ID associated with the instructor.
   * @returns {Object|null} - A promise that resolves to the instructor object or null if not found.
   * @throws {Error} - Throws an error if the operation fails.
   */
  static async getInstructorByUserId(userId) { // <--- ADDED THIS METHOD
    try {
      const instructor = await InstructorRepository.getInstructorByUserId(userId);
      return instructor; // Can be null if not found
    } catch (error) {
   ...
   */
  static async getInstruvtorCourses(insId) { // Typo: Should be getInstructorCourses
    try {
